import {
  countConsecutiveShortDays, integrateLuxHours, jstDayStart,
  jstHour, judgeDailyLight, LuxSample,
...
const at = (h: number, m = 0) =>
  new Date(Date.UTC(2026, 7, 23, h - 9, m, 0));

// 10分間隔でluxサンプルを生成
function series(startH: number, endH: number, lux: (h: number) => number): LuxSample[] {
  const out: LuxSample[] = [];
...
ok("jstDayStart が JST 00:00 を返す", jstDayStart(at(15)).getTime() === at(0).getTime());
...
console.log("\n--- 積算（台形則） ---");
{
  ...
  ok("信頼できる", r.reliable);
  ok("分母は日中12時間", Math.abs(r.windowHours - 12) < 0.1, `→ ${r.windowHours}`);
}
{
  // 日中に6時間の欠測（6〜12時が抜ける）
...
}
{
  ...
  ok("夜間欠測でも積算は正しい", Math.abs(r.luxHours - 12000) < 100, `→ ${r.luxHours}`);
}
{
...
{
  const v = judgeDailyLight({ now: at(10), samples: series(0, 10, () => 100), ...base });
  ok("15時前は判定しない", v.kind === "none");
}
{
  // 15時までずっと暗い（100lux）→ 1,500 lux·h、目標の15%
  const v = judgeDailyLight({ now: at(15), samples: series(0, 15, () => 100), ...base });
  ok("15時に不足見込みなら警告", v.kind === "warning", `→ ${v.kind}`);
}
{
  // 15時までに2000lux×15h = 30,000 lux·h → 十分
  const v = judgeDailyLight({ now: at(15), samples: series(0, 15, () => 2000), ...base });
  ok("15時に順調なら黙る", v.kind === "none", `→ ${v.kind}`);
}
{
  const v = judgeDailyLight({ now: at(15), samples: series(0, 15, () => 100), ...base, warnedToday: true });
  ok("同じ枠で二重に警告しない", v.kind === "none");
}
{
  const v = judgeDailyLight({ now: at(20), samples: series(0, 20, () => 100), ...base, warnedToday: true });
  ok("20時に未達なら確定通知", v.kind === "shortfall", `→ ${v.kind}`);
}
{
  // 15時まで暗く、その後明るい場所へ移動 → 目標達成
  const moved = [...series(0, 15, () => 100), ...series(15, 20, () => 4000)];
  const v = judgeDailyLight({ now: at(20), samples: moved, ...base, warnedToday: true });
  ok("警告後に達成したら回復通知", v.kind === "recovered", `→ ${v.kind}`);
}
{
  const v = judgeDailyLight({ now: at(20), samples: series(0, 20, () => 2000), ...base });
  ok("警告していない達成日は黙る", v.kind === "none", `→ ${v.kind}`);
}
{
  const v = judgeDailyLight({ now: at(20), samples: series(0, 20, () => 100), ...base, closedToday: true });
