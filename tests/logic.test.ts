// Layer 1 + Layer 2 のロジック検証（実行: npx tsx tests/logic.test.ts）
import {
  applyMedianFilter,
  toComfortScores,
  toScore,
...
const SOIL_OK = Math.round(
  SOIL_CALIBRATION.airValue -
    0.55 * (SOIL_CALIBRATION.airValue - SOIL_CALIBRATION.waterValue),
);
...
assert(
  "水分55%(快適) → 100",
  toScore(55, profile.moisture) === 100,
  `got ${toScore(55, profile.moisture)}`,
);
assert(
  "水分20%(注意境界) → 50",
  toScore(20, profile.moisture) === 50,
  `got ${toScore(20, profile.moisture)}`,
);
assert("水分10%(危険境界) → 0", toScore(10, profile.moisture) === 0);
assert(
  "水分90%(過湿・注意側) → 25",
  toScore(90, profile.moisture) === 25,
  `got ${toScore(90, profile.moisture)}`,
);
...
assert(
  "バグ値1回は中央値で無視される",
  Math.abs(f1.moisture_pct - soilAdcToPercent(1900)) < 0.01,
  `got ${f1.moisture_pct}`,
);

console.log("--- Layer 2: 感情判定 ---");
const fGood = applyMedianFilter([], normal);
const s1 = evaluateEmotion(
  toComfortScores(fGood, profile),
  fGood,
  comfortMid,
  [],
);
assert(
  "快適 → 満足",
  s1.emotion === "満足" && s1.complaint === null,
  JSON.stringify(s1),
);
...
const pastLogs = Array.from({ length: 10 }, (_, i) => ({
  emotion: "不満",
  complaint: "水分不足",
  created_at: new Date(Date.now() - (i === 9 ? 49 : i) * 3600 * 1000)
    .toISOString(),
}));
const s3 = evaluateEmotion(dryScores, fDry, comfortMid, pastLogs);
assert(
  "水分不足が2日以上継続と判定される",
  s3.duration_hours >= 48,
  `duration=${s3.duration_hours}`,
);
assert(
  "2段階エスカレートして苛立ちに到達",
  s3.emotion === "苛立ち",
...
assert(
  "湿度55%(快適) → 100",
  toScore(55, profile.humidity) === 100,
  `got ${toScore(55, profile.humidity)}`,
);
assert(
  "湿度25%(乾燥・注意境界) → 50",
  toScore(25, profile.humidity) === 50,
  `got ${toScore(25, profile.humidity)}`,
);
assert(
  "湿度90%(多湿・注意境界) → 50",
  toScore(90, profile.humidity) === 50,
  `got ${toScore(90, profile.humidity)}`,
);
assert(
  "中央値フィルタが湿度も通す",
...
const sDryAir = evaluateEmotion(
  toComfortScores(fDryAir, profile),
  fDryAir,
  comfortMid,
  [],
);
assert(
  "空気が乾燥 → 主訴=湿度低すぎ",
  sDryAir.complaint === "湿度低すぎ",
  JSON.stringify(sDryAir),
);

const humidAir = { soil_adc: SOIL_OK, temp: 24, humidity: 95, lux: 2000 };
const fHumidAir = applyMedianFilter([], humidAir);
const sHumidAir = evaluateEmotion(
  toComfortScores(fHumidAir, profile),
  fHumidAir,
  comfortMid,
  [],
);
assert(
  "空気が多湿 → 主訴=湿度高すぎ",
  sHumidAir.complaint === "湿度高すぎ",
  JSON.stringify(sHumidAir),
);
...
assert(
  "湿度が快適レンジなら主訴なし(満足)",
  sHC.emotion === "満足" && sHC.complaint === null,
  JSON.stringify(sHC),
);
...
const soilMildDry = Math.round(
  SOIL_CALIBRATION.airValue -
    0.30 * (SOIL_CALIBRATION.airValue - SOIL_CALIBRATION.waterValue),
); // 約30% = 水分やや不足(低urgency)
const bothMild = { soil_adc: soilMildDry, temp: 24, humidity: 30, lux: 2000 };
const fBoth = applyMedianFilter([], bothMild);
const sBoth = evaluateEmotion(
  toComfortScores(fBoth, profile),
  fBoth,
  comfortMid,
  [],
);
assert(
  "水分やや不足+空気乾燥(同urgency) → 水分不足を優先",
  sBoth.complaint === "水分不足",
  JSON.stringify(sBoth),
);

console.log("--- 状態遷移の検出（通知疲労対策） ---");
assert(
  "同じ状態の継続は通知しない",
  hasTransitioned(s2, { emotion: s2.emotion, complaint: s2.complaint }) ===
    false,
);
assert(
  "感情がエスカレートしたら通知する",
